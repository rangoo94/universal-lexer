...
function buildToken (token) {
  ...
  const tooltip = `<span class="tooltip"><strong>${token.type} ${position}</strong><span>${pretty(token.data)}</span></span>`

  return `<span class="code code--${token.type}">${tooltip}${code}</span>`
}
